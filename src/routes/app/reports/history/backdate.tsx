...
import { ArrowLeft, Save, Calendar as CalendarIcon, Clock } from "lucide-solid";
...
export default function BackdateTransaction() {
	...
	const handleSave = async (e: Event) => {
		...
		try {
			const parsedTotal =
				Number.parseInt(total().replaceAll(/\D/g, ""), 10) || 0;
			if (parsedTotal <= 0) throw new Error("Total tidak valid");

			// Robust ID and professional Receipt Number
			const transactionId = crypto.randomUUID();
			const receiptSuffix = Math.random().toString(36).substring(2, 7).toUpperCase();
			const receiptNumber = `BD-${receiptSuffix}`;
			...
			await db.transactions.add({
				...
				originalAmount: parsedTotal, // Standard requirement
				cogsTotal: 0,
		...
		<div class="flex flex-col min-h-screen bg-background pb-24">
			<div class="flex items-center gap-3 px-5 pt-6 pb-4 bg-background border-b border-border/40 sticky top-0 z-10 backdrop-blur-xl">
				...
				<div>
					...
					<span class="text-xs font-black text-muted-foreground uppercase tracking-[0.12em] mt-1 block">
						Catat Transaksi Masa Lalu
					</span>
				</div>
			</div>
					...
					>
						<div class="flex flex-col gap-2">
							...
							<input
								...
								value={total()}
								onInput={(e) =>
									setTotal((e.target as HTMLInputElement).value)
								}
								placeholder="0"
							/>
						</div>
						...
						<div class="grid grid-cols-2 gap-4">
							<div class="flex flex-col gap-2">
								<label
									class="text-sm font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1"
								>
								...
								<button
									...
									class="h-14 w-full rounded-2xl border border-border/80 bg-background px-3 flex flex-col justify-center text-left hover:border-primary/40 transition-all select-none"
								>
							...
							<div class="flex flex-col gap-2">
								...
								<input
									...
									value={timeStr()}
									onInput={(e) =>
										setTimeStr((e.target as HTMLInputElement).value)
									}
								/>
							</div>
						</div>
