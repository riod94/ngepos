  ...
  return (
    <div class="flex flex-col min-h-screen bg-background pb-24">
      ...
      <div class="p-5 flex-1">
        <div class="bg-card p-6 rounded-[28px] border border-border/60 shadow-sm">
          <form id="backdateForm" onSubmit={handleSave} class="flex flex-col gap-5">
            <div class="flex flex-col gap-2">
              <label class="text-[12px] font-black uppercase tracking-widest text-muted-foreground">
                Total Pendapatan (Rp)
              </label>
              <input
                id="backdate-total"
                required
                type="number"
                class="h-16 w-full rounded-2xl border-2 border-border/60 bg-background px-4 font-black text-[28px] text-foreground focus:outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all"
                value={total()}
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="flex flex-col gap-2">
                <label for="backdate-date" class="text-[12px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                ...
                <input
                  ...
                  required
                  type="date"
                  class="h-14 w-full rounded-2xl border border-border/80 bg-background px-3 font-semibold text-[15px] focus:outline-none focus:ring-2 focus:ring-primary/20"
                  value={dateStr()}
                  onInput={(e) => setDateStr((e.target as HTMLInputElement).value)}
                />
              </div>
              <div class="flex flex-col gap-2">
                <label for="backdate-time" class="text-[12px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                  <Clock size={14} /> Jam
                </label>
                <input
                  id="backdate-time"
            ...
            <div class="flex flex-col gap-2">
              <label class="text-[12px] font-black uppercase tracking-widest text-muted-foreground">Metode Pembayaran</label>
              <div class="flex items-center gap-3 bg-muted/30 p-1.5 rounded-2xl">
                <button
                  type="button"
                  onClick={() => setMethod('CASH')}
                  class={`flex-1 h-12 rounded-xl font-black text-[15px] transition-all ${method() === 'CASH' ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground hover:bg-muted'}`}
                >
                  Tunai
                </button>
                <button
                  ...
                  class={`flex-1 h-12 rounded-xl font-black text-[15px] transition-all ${method() === 'QRIS' ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground hover:bg-muted'}`}
                >
                  QRIS
                </button>
              </div>
            ...
            <Button
              type="submit"
              disabled={isSaving()}
              class="w-full h-16 mt-4 rounded-2xl font-black text-[17px] bg-foreground text-background shadow-lg hover:bg-foreground/90 flex items-center justify-center gap-2 border-none"
            >
              {isSaving() ? (
