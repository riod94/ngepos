...
export default function Expenses() {
  const [period, setPeriod] = createSignal<"HARI_INI" | "BULAN_INI" | "SEMUA">("BULAN_INI");
  const [expenses, { refetch }] = createResource(period, async (p) => {
  ...
  const [formCategory, setFormCategory] = createSignal<ExpenseCategory>("operasional");
      ...
      <div class="px-5 pt-6 pb-4 bg-background border-b border-border/40 sticky top-0 z-10 backdrop-blur-xl">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="font-black text-[22px] tracking-tight">Pengeluaran</h1>
              <span class="text-[11px] font-black text-muted-foreground uppercase tracking-widest">Catatan Biaya Operasional</span>
            </div>
          </div>
        ...
        <SheetContent position="bottom" class="h-auto max-h-[90vh] rounded-t-[32px] flex flex-col p-0 border-none shadow-[0_-20px_60px_rgba(0,0,0,0.15)]">
          ...
          <form onSubmit={handleSave} class="flex flex-col gap-5 p-5 overflow-y-auto">
            ...
            <div class="flex items-center justify-between p-4 bg-amber-50 rounded-2xl border border-amber-200">
              ...
              >
                <div class={`w-5 h-5 bg-white rounded-full absolute top-1 transition-all shadow-sm ${formIsBackdated() ? "left-6" : "left-1"}`} />
              </button>
            </div>
            ...
            >
              {isSaving() ? (
                ...
                isEditing() ? "Perbarui Pengeluaran" : "Simpan Pengeluaran"
              )}
            </Button>
          </form>
        </SheetContent>
