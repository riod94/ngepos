...
- Business logic: useCheckout encapsulates transaction creation, inventory deduction, and loyalty post-processing.
- Data access: Dexie-backed local database (db.ts) persists transactions and items; syncService coordinates background synchronization.
...
```mermaid
graph TB
UI["CartFloatingButton.tsx<br/>Payment UI & orchestration"] --> Hook["useCheckout.ts<br/>Transaction creation"]
Hook --> DBLocal["db.ts<br/>Dexie local store"]
Hook --> Sync["syncService.ts<br/>Background sync"]
Sync --> API["/api/sync/index.ts<br/>Server upsert"]
UI --> Receipt["receipt/[id].tsx<br/>Receipt rendering"]
UI --> Loyalty["loyalty.ts<br/>Stamp/Reward logic"]
...
## Core Components
- useCheckout: Centralized checkout logic that creates transactions, deducts inventory, computes costs, and triggers background sync.
...
- syncService: Background sync of PENDING transactions to the backend API.
...
- loyalty store: Handles stamp eligibility, progress, and reward claiming after checkout.
...
The checkout pipeline is designed for offline-first operation:
- Local transaction creation: useCheckout writes to Dexie with PENDING status.
- Inventory and cost-of-goods updates: COGS computed per-item and per-recipe, stock decremented, inventory logs recorded.
- Loyalty post-processing: After commit, stamps and rewards are updated if eligible.
- Background sync: syncService periodically sends PENDING transactions to the backend; upon success, status is updated to SYNCED.
...
### useCheckout Hook
Responsibilities:
- Validates cart state and snapshot.
- Computes discounts and loyalty reward contribution.
...
- Updates loyalty stamps and rewards after commit.
...
### Loyalty Integration Post-Checkout
- Eligibility checks: minimum transaction threshold, promo allowance, and excluded products.
- Stamps: recorded per transaction if eligible; progress computed considering expiry windows.
...
### Offline-First and Sync Queue Management
- Local state: transactions created with PENDING status.
- Background sync: debounced push of PENDING transactions and expenses to backend API.
...
Resp --> |No| Retry["Error logged (retry on next trigger)"]
...
- Network failures during sync: PENDING transactions persist locally; retry occurs on next trigger; verify auth token presence.
...
## Conclusion
The checkout system integrates a robust offline-first model with immediate local transaction creation, precise inventory and COGS handling, flexible payment methods, and seamless background synchronization. The useCheckout hook centralizes validation, inventory deduction, and loyalty post-processing, while UI components provide clear feedback and navigation. The sync service and backend API ensure eventual consistency and conflict-free reconciliation.